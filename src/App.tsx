...
function App() {
  ...
  async function handleConvert() {
    ...
    const passRows: Array<{
      ...
      deltaBytes: number
      ratio: number
      nextVideoBitrateBps: number
      elapsedMs: number
      downscaleWidth?: number
      downscaleHeight?: number
    }> = []

    let didOpenConsoleGroup = false
    ...
    try {
      ...
      console.groupCollapsed('[auto] compression')
      didOpenConsoleGroup = true
      console.log({
        fileName: file.name,
        ...
        durationSeconds,
        settings: {
          targetBytes: AUTO_TARGET_BYTES,
          audioBitrateBps: AUDIO_BITRATE_BPS,
          bppThreshold: BPP_THRESHOLD,
      ...
      console.log('[auto] input analysis', {
        displayWidth,
        displayHeight,
        estimatedFps,
        hasAudio,
      ...
      console.log('[auto] budgets', {
        overheadBytes,
        overhead: formatBytes(overheadBytes),
        audioBytesBudget,
        audioBudget: formatBytes(audioBytesBudget),
        videoBytesBudget,
        videoBudget: formatBytes(videoBytesBudget),
        initialVideoBitrateBps: videoBitrateBps,
        pixelsPerSecond,
        bitsPerPixel,
        isOver1080p,
        shouldDownscale,
      ...
      for (let attempt = 1; attempt <= AUTO_MAX_ATTEMPTS; attempt += 1) {
        ...
        const elapsedMs = Math.round(performance.now() - passStart)

        const ratio = AUTO_TARGET_BYTES / sizeBytes

        let nextVideoBitrateBps = videoBitrateBps
        if (sizeBytes > AUTO_TARGET_BYTES) {
          ...
          nextVideoBitrateBps = Math.max(reducedBitrate, MIN_VIDEO_BITRATE_BPS)
        }
        ...
        passRows.push({
          ...
          videoBitrateBps,
          sizeBytes,
          sizeMb: Number((sizeBytes / 1_000_000).toFixed(2)),
          targetBytes: AUTO_TARGET_BYTES,
          deltaBytes: sizeBytes - AUTO_TARGET_BYTES,
          ratio: Number(ratio.toFixed(3)),
          nextVideoBitrateBps,
          elapsedMs,
          ...(downscaleWidth !== undefined ? { downscaleWidth } : {}),
          ...(downscaleHeight !== undefined ? { downscaleHeight } : {}),
        })

        console.log(`[auto] pass ${attempt}/${AUTO_MAX_ATTEMPTS}`, {
          videoBitrateBps,
          sizeBytes,
          size: formatBytes(sizeBytes),
          target: formatBytes(AUTO_TARGET_BYTES),
          ratio,
          nextVideoBitrateBps,
          elapsedMs,
        })
        ...
        videoBitrateBps = nextVideoBitrateBps
      }
      ...
      if (didOpenConsoleGroup) console.groupEnd()
